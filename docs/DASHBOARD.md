...
## 🖥️ ทางลัด: Mac Mini + Quick Tunnel (ไม่ต้องมี VPS/โดเมน)

เหมาะกับการ **รันบอทบน Mac เฉพาะตอนเปิดดรอป** (ดรอปขายหมดเร็ว) แล้วให้ตัวเอง + วีทูปเบอร์เข้าดูจากที่ไหนก็ได้แบบชั่วคราว

1. ติดตั้ง cloudflared ครั้งเดียว: `brew install cloudflared`
2. ตั้งใน `.env`: `ADMIN_PORT=8787` และ `ADMIN_TOKEN=<token แข็งแรง>` — สร้าง token ด้วย `openssl rand -hex 24`
3. เวลาจะเปิดดรอป รันปุ่มเดียว:
   ```bash
   ./scripts/start-tunnel.sh
   ```
   มันจะเปิดบอท + tunnel แล้วพิมพ์ลิงก์ `https://xxxx.trycloudflare.com/?token=...` — ส่งลิงก์นั้นให้ตัวเอง + วีทูปเบอร์
4. ขายจบ กด **Ctrl+C** ปิดทั้งบอทและ tunnel

**ข้อจำกัด (โอเคสำหรับ 2 คน ใช้ชั่วคราว):**
- URL เปลี่ยนทุกครั้งที่รัน (ส่งลิงก์ใหม่ทุกดรอป)
- auth มีแค่ `ADMIN_TOKEN` (ยังไม่มี Cloudflare Access เพราะไม่มีโดเมน) → **ตั้ง token ยาว/สุ่ม** อย่าแชร์เกิน 2 คน
- token ไม่ค้างใน address bar (ระบบเก็บเป็น HttpOnly cookie ให้หลังเปิดครั้งแรก)

> อยากได้ **URL ถาวร + ล็อกอินด้วยอีเมล** (ปลอดภัยกว่า) → ซื้อโดเมนถูกๆ ใส่ Cloudflare แล้วทำ Named Tunnel + Access ตามหัวข้อ 2–3 ด้านล่าง
